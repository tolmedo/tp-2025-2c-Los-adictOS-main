...
static t_deserializado_master* _deserializar_desalojo_query(void* payload) {
    t_deserializado_master* deserializado = malloc(sizeof(*deserializado));
    int offset = 0;
    
    deserializado->header = DESALOJO_QUERY;
    deserializado->payload.desalojo_query.query_id = obtener_un_entero(payload, &offset);
    deserializado->payload.desalojo_query.program_counter = obtener_un_entero(payload, &offset);
...
t_deserializado_master* deserializar_mensaje_master(t_header header, void* payload) {
    ...
    switch (header) {
        ...
        case DESALOJO_QUERY:
            deserializado = _deserializar_desalojo_query(payload);
            break;
